...
dt["fbs"] = dt["fbs"].replace({
    "มากกว่า 12ไม่เกิน 120 mg/dL": 1,
    "มากกว่า 120 mg/dL": 1,
    "ไม่เกิน 120 mg/dL": 0
})

dt["restecg"] = dt["restecg"].replace({
    "ปกติ": 0,
    "มีความผิดปกติของคลื่น ST-T": 1,
    "มีภาวะหัวใจห้องล่างซ้ายโต ตามเกณฑ์ของ Estes": 2
})

dt["exang"] = dt["exang"].replace({
    "มี": 1,
    "ไม่มี": 0
})

dt["slope"] = dt["slope"].replace({
    "ชันขึ้น (Upsloping)": 0,
    "ราบ (Flat)": 1,
    "ชันลง (Downsloping)": 2
})

dt["thal"] = dt["thal"].replace({
    "ข้อมูลผิดพลาดหรือไม่มีข้อมูล": 0,
    "ผิดปกติแบบคงที่": 1,
    "ผิดปกติแบบคงที่ (Fixed Defect)": 1,
    "ปกติ (Normal)": 2,
