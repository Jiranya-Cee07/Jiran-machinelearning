...
st.info(
    ...
    **Dataset : Heart Disease**
    ...
    ชุดข้อมูลนี้ใช้สำหรับทำนายความเสี่ยงของโรคหัวใจ
    """
)
...
with st.sidebar:
    ...
    chol = st.slider('ระดับคอเลสเตอรอลในเลือด(mg/dL)(chol)', 1, 600, 150)
    fbs = st.selectbox('ระดับน้ำตาลในเลือดหลังอดอาหาร(fbs)',('มากกว่า 120 mg/dL','ไม่เกิน 120 mg/dL'))
    ...
    thalach = st.slider('อัตราการเต้นของหัวใจสูงสุดที่วัดได้(thalach)', 1, 250, 60)
    exang = st.selectbox('อาการเจ็บหน้าอกจากการออกกำลังกาย(exang)',('มี','ไม่มี'))
    oldpeak = st.slider('ระดับการลดลงของช่วง ST ที่เกิดจากการออกกำลังกายเมื่อเทียบกับขณะพัก(oldpeak)', 0.0, 10.00, 5.0)
